...
public final class CppLinkAction extends AbstractAction implements CommandAction {
  ...
  private Spawn createSpawn(ActionExecutionContext actionExecutionContext)
    ...
    try {
      ...
      System.err.printf(
          "[bazel:src/main/java/com/google/devtools/build/lib/rules/cpp/CppLinkAction.java] command line: %s, inputs: %s, outputs: %s, os: %s, tools: %s \n",
...
//            if (OS.getCurrent() == OS.DARWIN) {
        ...
        System.err.printf(
            "[bazel:src/main/java/com/google/devtools/build/lib/rules/cpp/CppLinkAction.java] output library - execPath = %s, fileName = %s, dirName = %s\n",
        ...
        if (arguments.getBytes().length > upperBoundaryOfArguments) {
          System.err.printf(
              "[bazel:src/main/java/com/google/devtools/build/lib/rules/cpp/CppLinkAction.java] need to split arguments to avoid the issue of \"error=7, Argument list too long\" on macOS 10.13.6\n");
          ...
          final List<String> objectFiles = commandLine.subList(indexOfOutputLibrary + 1,
              commandLine.size());
          final List<ObjectFile> objects = objectFiles.stream()
              .map(strF -> new ObjectFile(strF, strF.substring(strF.lastIndexOf("."))))
              .collect(Collectors.toList());
          final Map<String, List<ObjectFile>> postfixToObjectFileMap = objects.stream()
          ...
          if (unSupportedPostfixes.size() > 0) {
            String message = String.format(
                ...
                unSupportedPostfixes);
            DetailedExitCode code = createDetailedExitCode(message,
          ...
          final List<String> refinedCommands = new ArrayList<>(
              commandLine.subList(0, indexOfOutputLibrary + 1));
          refinedCommands.addAll(intermediateToSubLibraries.keySet());
          System.err.println(refinedCommands);
