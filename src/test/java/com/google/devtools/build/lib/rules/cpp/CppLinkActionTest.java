...
public class CppLinkActionTest extends BuildViewTestCase {
    ...
    @Test
    public void testCPPCrackMacOSLink() throws Exception {
        RuleContext ruleContext = createDummyRuleContext();
        FeatureConfiguration featureConfiguration =
                CcToolchainTestHelper.buildFeatures(
                                "action_config {",
        ...
        linkAction.setMock(true);

        final ImmutableMap<String, String> clientEnv = ImmutableMap.<String, String>builder()
                .build();
        final String origin = new String(Files.readAllBytes(Paths.get("src/test/java/com/google/devtools/build/lib/rules/cpp/CppLinkActionTest.macOScrack.txt")));
        final ImmutableList<String> commandLine = ImmutableList.copyOf(Lists.newArrayList(Splitter.on(" ").split(origin)).parallelStream().map(s -> {
            if (s.endsWith(",")) {
    ...
    @Test
    public void testTensorflowCrackMacOSLink() throws Exception {
        RuleContext ruleContext = createDummyRuleContext();
        FeatureConfiguration featureConfiguration =
                CcToolchainTestHelper.buildFeatures(
                                "action_config {",
        ...
        final ImmutableMap<String, String> clientEnv = ImmutableMap.<String, String>builder()
                .build();
        final String origin = new String(Files.readAllBytes(Paths.get("src/test/java/com/google/devtools/build/lib/rules/cpp/CppLinkActionTest.tensorflow2.9.txt")));
        final ImmutableList<String> commandLine = ImmutableList.copyOf(Lists.newArrayList(Splitter.on(" ").split(origin)).parallelStream().map(s -> {
            if (s.endsWith(",")) {
                return s.substring(0, s.length() - 1);
            }
